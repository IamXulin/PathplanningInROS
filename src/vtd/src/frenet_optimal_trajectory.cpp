...
#define TARGET_SPEED 5.0  // target speed [m/s]
...
bool FrenetOptimalTrajectory::check_collision(FrenetPath path,
                                              const Vec_Poi ob,
                                  const vector<VehicleState>& dynamic_obstcle_list_) {
  for (auto point : ob) {
  ...
  for (auto point : dynamic_obstcle_list_) {
    for (unsigned int i = 0; i < path.x.size(); i++) {
      double x=path.x[i];
      double y=path.y[i];
      double x1=point.x;
      double y1=point.y;
      double vx=point.velocity*cos(point.heading);
      double vy=point.velocity*sin(point.heading);
      double ax=point.acceleration*cos(point.heading);
      double ay=point.acceleration*sin(point.heading);
      double x2=x1+vx*path.t.back() + 0.5*ax*pow(path.t.back(),2);
      double y2=y1+vy*path.t.back() + 0.5*ay*pow(path.t.back(),2);
      
      double v1[2]={x1-x , y1-y};
      double v2[2]={x2-x1 , y2-y1};
      double v3[2]={x2-x , y2-y};
      double min_d;
      if ((v1[0]*v2[0]+v1[1]*v2[1])*(v3[0]*v2[0]+v3[1]*v2[1])>0){
        min_d=min(sqrt(v1[0]*v1[0]+v1[1]*v1[1]) , sqrt(v3[0]*v3[0]+v3[1]*v3[1]));
      }else{
        min_d=(v1[0]*v2[1]+v1[1]*v2[0])/(sqrt(v2[0]*v2[0]+v2[1]*v2[1]));
      }

      if (min_d <= ROBOT_RADIUS) {
        //std::cout<<"lllllllllllllllllllllllllllllllllllllll"<<std::endl;
        return false;
      }
    }
                                          ...
                                          const vector<VehicleState>& dynamic_obstcle_list_) 
{
  ...
  for (FrenetPath path : path_list) 
  {
    if (//path.max_speed < MAX_SPEED && path.max_accel < MAX_ACCEL &&
        path.max_curvature < MAX_CURVATURE && check_collision(path, ob,dynamic_obstcle_list_)) 
    {
...
FrenetPath FrenetOptimalTrajectory::frenet_optimal_planning(
    ...
    Vec_Poi ob, const vector<VehicleState>& dynamic_obstcle_list_) 
{
    // 01 
    // 获取采样轨迹数组
    // 02
    ...
    Vec_Path save_paths = check_paths(fp_list, ob,dynamic_obstcle_list_);
    ...
    // std::cout<<"=================================================="<<std::endl;
    // for (int k=0;k<final_path.y.size();k++){
      
    //   std::cout<<"xx=="<<final_path.x[k]<<"  yy=="<<final_path.y[k]<<std::endl;
    // }
    if (final_path.x.empty()){
      std::cout<<"无法找到合适路径"<<std::endl;
    }
    return final_path;
};
