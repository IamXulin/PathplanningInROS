...
void DIJKSTRA::Advertise_start(int init_x,int init_y){
    start_x=init_x;
    start_y=init_y;
    full_MarkerArray(start_x,start_y,-2,1.0,1.0,0.3,1.0,5.0,0.0,1.0,"start");
    pub_SG.publish(start);
}
void DIJKSTRA::Advertise_goal(int end_x,int end_y){
    goal_x=end_x;
    goal_y=end_y;
...
void DIJKSTRA::Advertise_process(){
    pub.publish(Process);
}

void DIJKSTRA::full_PathArray(int x,int y){
...
void DIJKSTRA::full_MarkerArray(int x,int y,int id,double scale_x,double scale_y,
...
bool DIJKSTRA::Iscollision(int next_x,int next_y){
    if (my_map[next_x][next_y]==1) return true;
    return false;
}

void DIJKSTRA::generate_obs(){
...
void DIJKSTRA::pub_obs(){
    pub.publish(Obs_gather);
    
}


void DIJKSTRA::full_path(){
